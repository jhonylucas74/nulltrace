# Nexus HTTP Server and VM Bootstrap

This document describes the Nexus HTTP server VM, the `httpd` bin program, and the VM bootstrap pattern used to auto-start processes when a VM is loaded.

---

## Overview

The cluster runs a dedicated HTTP server VM (Nexus) for testing the HTTP protocol. The server listens on port 80 and serves files from `/var/www` (file-based, no hardcoded routes). VMs can declare startup processes in `/etc/bootstrap`; when a VM is loaded, the cluster reads this file and spawns each listed program automatically.

---

## Nexus HTTP Server VM

### Purpose

Nexus is a system VM that runs an HTTP server for protocol testing. It is similar to Haru's VM but dedicated to serving HTTP traffic. Clients (e.g. Haru's terminal) can connect to Nexus and fetch pages over the HTTP protocol.

### Creation

- **Owner:** The `webserver` player (seeded via `seed_webserver()` in `player_service`).
- **Hostname:** `nexus-server`
- **DNS name:** `nexus.local` (when DNS resolution is available).
- **Creation flow:** In `load_game_vms` (main.rs), if the webserver player has no VM, the cluster creates one with `manager.create_vm(config)`, creates `/var/www`, writes seed files (index.ntml, robot.txt, about.ntml, 404.ntml), and writes `/etc/bootstrap` with `httpd /var/www` so the server starts on load.

### Bootstrap

The Nexus VM has `/etc/bootstrap` containing:
...
- Created manually or during VM creation (e.g. Nexus gets it when created).
...
```mermaid
flowchart TB
    subgraph create [Nexus VM Creation]
        seedWebserver[seed_webserver player]
        createVM[manager.create_vm]
        mkdirWWW[Create /var/www]
        writeFiles[Write index.ntml robot.txt about.ntml 404.ntml]
...
From another VM (e.g. Haru's terminal), connect to Nexus and fetch pages:

- Use the Nexus VM's IP (shown in cluster logs) until DNS resolution for `nexus.local` is available.
- Example: `net.connect("<nexus_ip>", 80)`, send GET request, parse response.
